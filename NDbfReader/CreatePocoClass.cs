...
namespace NDbfReaderEx
{
    public class CreatePocoClass                                              // new from 1.3 version
    {
        ...
        public void CreateCS(string className, IEnumerable<IColumn> columns, string baseClass = "", bool overwriteFile = false)   // IColumn[] columns
        {
            ...
            for (int i = 0; (i < templateText.Length); i++)
            {
                if (!IsRemarkedLine(templateText[i]) && templateText[i].Contains(classNameText))
                {
                    ...
                    else
                    {
                        if (lClassDefinition)
                        {
                            templateText[i] = templateText[i].Replace(classNameText, $"{className} : {baseClass}");
                            lClassDefinition = false;
                        }
                        else
                        {
                            templateText[i] = templateText[i].Replace(classNameText, className);
